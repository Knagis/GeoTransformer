...
Pele_ ( Elina &amp; Elviss )</groundspeak:text>
  ...
  <wpt lat=""56.990978"" lon=""23.523668"">
    ...
    <type>Waypoint|Parking Area</type>
            ...
            var ser = gdoc.Serialize(GeoTransformer.Gpx.GpxSerializationOptions.Compatibility);

            var logOrg = xdoc.Descendants(XmlExtensions.GeocacheSchema_1_0_1 + "log").First();
            var logNew = ser.Descendants(XmlExtensions.GeocacheSchema_1_0_1 + "log").First();
            Assert.AreEqual(logOrg.ToString(), logNew.ToString(), true, "Log is not serialized correctly.");
        }
